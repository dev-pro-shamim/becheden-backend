...
function readLogFile(): {
    ...
    try {
        if (fs.existsSync(responseLogFilePath)) {
            const responseLogData = fs.readFileSync(
                responseLogFilePath,
                'utf8',
            );

            // Robustly handle multiple JSON objects on the same line or concatenated
            const normalizedData = responseLogData.replace(
                /\}[\s]*\{/g,
                '}\n{',
            );
            ...
            normalizedData.split('\n').forEach((entry) => {
                ...
                try {
                    ...
                    console.error(
                        'Error parsing response time log entry:',
                        err,
                    );
                }
            });
    ...
    try {
        if (fs.existsSync(errorLogFilePath)) {
            ...
            // Robustly handle multiple JSON objects on the same line or concatenated
            const normalizedErrorData = errorLogData.replace(
                /\}[\s]*\{/g,
                '}\n{',
            );
...
async function generateCpuUsageHtml(): Promise<string> {
    return new Promise((resolve) => {
        osUtils.cpuUsage((usage: number) => {
            const totalCores = os.cpus().length;
            const cpuUsagePercentage = usage * 100;
            const usedCores = Math.ceil(
                (cpuUsagePercentage / 100) * totalCores,
            );
            const idleCores = totalCores - usedCores;
...
export function getMonitorStats() {
    ...
    const avgResponseTime =
        totalRequests > 0
            ? (
                  responseTimeLogs.reduce(
                      (acc, log) => acc + log.responseTime,
                      0,
                  ) / totalRequests
              ).toFixed(2)
            : '0';
