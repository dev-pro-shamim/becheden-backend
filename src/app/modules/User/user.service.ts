...
type TSignupFilePayload =
  | Record<string, Express.Multer.File[]>
  | Express.Multer.File[]
  | undefined;

const normalizeVendorFiles = (
  files: TSignupFilePayload,
): TVendorFileMap | undefined => {
  if (!files) {
    return undefined;
  }
  ...
  return files as TVendorFileMap;
};

// 1. createUserInDB
const createUserInDB = async (
  payload: IUser & TVendorSignupExtras,
  files?: TSignupFilePayload,
) => {
  ...
  if (!existingUser) {
    const { storeName, storeLocation, tradeLicenseNumber, ...userPayload } =
      payload;
    ...
    const vendorFiles = normalizeVendorFiles(files);
    ...
    if (newUser.role === ROLE.VENDOR) {
      ...
      try {
        ...
        if (storeImageUrl) {
          await deleteImageFromCloudinary(storeImageUrl);
        }
        if (tradeLicenseUrl) {
          await deleteImageFromCloudinary(tradeLicenseUrl);
        }
    ...
    try {
      await sendOtpEmail({ email: newUser.email, otp, name: newUser.name });
    } catch (error) {
      if (newUser.role === ROLE.VENDOR) {
        await VendorModel.deleteOne({ user: newUser._id });
